...
const CreditAmount = () => {
  ...
  const cureentBalanceHandler = (event) => {
    ...
    axios.put('http://localhost:8080/customer/credit/' + accountNumber + "/" + creditAmount).then((response) => {
    ...
    <div>
      <StyledComponet>
        <form onSubmit={cureentBalanceHandler}>
          ...
          <div class="form-group row">
            <label for="staticEmail" class="col-sm-2 col-form-label">Credit Amount</label>
            <div class="col-sm-10">
          ...
          {balance == -1 && <p>please Enter Valid Account Number</p>}
